"""
Trade ledger with stop-loss support, backed by DuckDB with parquet export on resolution.

Open trades live in /data/model_sl_trades.db. Stop-loss exits are written
immediately. On market resolution the remaining open trades are resolved and
all closed trades are exported to /data/trades/model_sl/ then deleted from DB.

Extra column vs base: exit_price — price at which a stop-loss position was closed.
"""
...
_store = TableStore(
    db_path=str(Path(settings.local_data_dir) / "model_sl_trades.db"),
    table_name="trades",
    create_sql=_CREATE_SQL,
    parquet_dir=Path(settings.local_data_dir) / "trades" / "model_sl",
)
